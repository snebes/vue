...
(function () {
    ...
    CKEDITOR.plugins.add('cdpNumberedList', {
        ...
        init: function (editor) {
            ...
            var plugin = CKEDITOR.plugins.cdpNumberedList;

            editor.addCommand('cdpNumberedList', {
                allowedContent: 'ol li',
                ...
                refresh: function (editor, path) {
                    var ol = path.contains('ol', 1);
                    this.setState(ol ? CKEDITOR.TRISTATE_ON : CKEDITOR.TRISTATE_OFF);
                }
            });

            editor.addCommand('cdpNumberedListItem', {
                contextSensitive: true,
                ...
                exec: function (editor) {
                    ...
                    plugin.formatListElement(editor, range);
    ...
    CKEDITOR.plugins.cdpNumberedList = {
        ...
        createListFromRange: function (editor, range) {
            var bm = range.createBookmark(),
                ...
                blocks = [],
                // Lists (ol,ul) that we may empty when moving blocks
                // to newly created ol. These lists should be removed.
                listsToCheck = [],
                lastBlockParent, ol, block;
            ...
            // first block.
            if (!ol) {
                ol = this.createListContainer(editor, blocks[0]);
            }
            ...
            moveBlocksToListContainer(ol, blocks, listsToCheck);
            ...
            mergeToFollowingListContainer(ol);
        ...
        removeListFromRange: function (editor, range) {
            ...
            var currentList = block.getAscendant('ol'),
        ...
        createListContainer: function (editor, block) {
            ...
            ol.setAttribute('class', 'cdp-numbered-list');
            ol.insertBefore(list ? secondList : block);
            ...
            if (list) {
                // Split might result in leaving: <ol></ol><ul></ul>...
                if (isBlockEmpty(list)) {
                    list.remove();

                    // It could also result in: <ol><li></li></ol><ul></ul>...
         ...
         * @param range
         */
        formatListElement: function (editor, range) {
            var bm = range.createBookmark(),
            ...
            while ((node = walker.next())) {
                if (isLi(node)) {
                    ...
                    if (child && child.is('#text')) {
                        newLabel = node.getDocument().createElement('div');
                        newLabel.setAttribute('class', 'label');
                        newLabel.setText('#.');
                        newLabel.appendTo(node);

                        newItem = node.getDocument().createElement('div');
                        newItem.setAttribute('class', 'list-item');
                        newItem.appendTo(node);

                        child.moveChildren(newItem);
                        child.remove();
                    } else if (child && child.is('div') && child.getAttribute('class') == 'label') {
                        newItem = node.getDocument().createElement('div');
                        newItem.setAttribute('class', 'list-item');
                        newItem.setText('Enter text...');
                        newItem.insertAfter(child);
                    } else if (child && child.is('div') && child.getAttribute('class') == 'list-item') {
                        newLabel = node.getDocument().createElement('div');
                        newLabel.setAttribute('class', 'label');
                        newLabel.setText('#.');
                        newLabel.insertBefore(child);
                    } else {
                        newLabel = node.getDocument().createElement('div');
                        newLabel.setAttribute('class', 'label');
                        newLabel.setText('#.');
    ...
    /**
     * @param {CKEDITOR.dom.element} ol       List container to which blocks will be appended
     * @param {CKEDITOR.dom.element} block   Blocks that will be transformed to li and appended to list container
     */
    function createListElement(ol, block) {
        ...
        if (block.is(listElementNames)) {
            ...
            var newBlock = block.getDocument().createElement('li'),
                newLabel = block.getDocument().createElement('div'),
                newContent = block.getDocument().createElement('div');

            newLabel.setAttribute('class', 'label');
            newLabel.appendTo(newBlock);
            newLabel.setText('#.');
            newContent.setAttribute('class', 'list-item');
            newContent.appendTo(newBlock);
            newContent.setText('Enter text...');

            block.moveChildren(newContent);
            newBlock.appendTo(ol);
            block.remove();
        }
    }
    ...
    function getWrappingList(block) {
        ...
        } else if (blockParent && blockParent.is(listElementNames)) {
    ...
    function handleEnterKey(editor) {
        ...
        cmd.on('exec', function (evt) {
            ...
            if (range.collapsed && range.checkEndOfBlock() && range.checkStartOfBlock()) {
                editor.execCommand('cdpNumberedList');
                evt.cancel();
            }
        });

        editor.on('afterCommandExec', function (evt) {
            if (evt.data.name == 'enter') { // && evt.data.togglCdpListBlock) {
                editor.execCommand('cdpNumberedListItem');
            }
        });
    }
    ...
    /**
     * Checks whether selection is in list item (but not deeper - e.g. in <li><p>^</p></li>).
     *
     * @param editor
     * @return {Boolean}
     */
    function isSelectionInListItem(editor) {
        ...
        return path.block && path.block.is(listElementNames) ||
            path.block.getParent() && path.block.getParent().is(listElementNames);
    }

    /**
     * Looks for list container that follows the passed one and if found moves children of the passed one to the
     * following one.
     *
     * @param {CKEDITOR.dom.element} ol
     */
    function mergeToFollowingListContainer(ol) {
        var nextNode = ol.getNext(isNotIgnored);

        if (nextNode && isOl(nextNode)) {
            ol.moveChildren(nextNode, true);
            ol.remove();
        }
    }
    ...
    /**
     * @param {CKEDITOR.dom.element}   ol           List container to which blocks will be appended
     * @param {CKEDITOR.dom.element[]} blocks       Blocks that will be transformed to li and appended to list container
     * @param {CKEDITOR.dom.element[]} listsToCheck Array to which lists containers (ul, ol) that might be emptied when
     *                                              moving blocks will be pushed.
     */
    function moveBlocksToListContainer(ol, blocks, listsToCheck) {
        var list, block, i;
    ...
    function refreshCallback() {
        ...
        return function (editor, path) {
            ...
            if (!listElement) {
                ...
                this.setState(listElement.is('li') ? CKEDITOR.TRISTATE_ON : CKEDITOR.TRISTATE_OFF);
            }
        };
    }
})();
